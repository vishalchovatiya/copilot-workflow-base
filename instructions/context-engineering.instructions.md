...
## Keep AI Context in Sync with Code

The `.github/` workflow (instructions, prompts, skills) is itself context that Copilot
loads. When code structure, a module boundary, or the runtime flow changes, update the
matching `.github` artifacts and the `README.md` in the same change — stale instructions
or playbooks make Copilot generate code against a system that no longer exists.
