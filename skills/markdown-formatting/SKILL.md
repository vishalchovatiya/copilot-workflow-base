...
description: "Format, write, review, or fix Markdown (.md) files per the CommonMark 0.31.2 spec with project conventions: hierarchical dotted section numbering, a mirror Table of Contents, aligned pipe tables with a leading row-number column, Mermaid-only diagrams sharing one theme, and math formulas in intuition -> formula -> breakdown layout. Use whenever the user creates, edits, modifies, reviews, or cleans up any Markdown file. Trigger phrases: 'edit this markdown', 'write a README', 'fix this md', 'fix the markdown', 'update the docs', 'format this .md', 'add a section to', 'review this markdown'. Also trigger whenever a task involves writing or changing content in a file ending in .md or .markdown."
---
...
- **Tables**: Format columns with aligned pipes and consistent cell padding so the raw Markdown stays well formatted and easy to edit by hand. Section numbering, the Table of Contents, and table alignment are all enforced by [`check_markdown.py`](../../scripts/check_markdown.py), which ships **inside this workflow bundle** at `scripts/check_markdown.py` (two directories up from this `SKILL.md`) — **not** in the workspace being edited. After editing any file, resolve the checker relative to this skill and run it with `--fix <file>` to renumber headings, rebuild the TOC, and align tables (the checker touches only heading lines, the TOC list, and table blocks — never other prose or `$…$` math). The bundle is loaded from one of two layouts, so do not hardcode a single path — locate the script before running it:
  - **Submodule layout** (vendored in a repo): `.github/shared/scripts/check_markdown.py`.
  - **Machine-wide layout** (loaded from User settings): the standalone clone, e.g. `~/copilot-workflow-base/scripts/check_markdown.py` (or wherever `chat.agentSkillsLocations` points).

  A portable way to find and run it regardless of layout:
  ...
  If neither location exists, the checker is not installed on this machine — apply the numbering, TOC, and table-alignment rules by hand instead of failing.
